...
普段はグロービス様のプロダクト開発のお手伝いをさせていただいています。

今回は、その日々の活動の中でプロジェクトに AIコードレビューを導入しましたので、そのお話をさせていただきます。
...
ものにより様々ですが、タイポなどの些末な指摘から、セキュリティやパフォーマンスの問題にしてもフィードバックをしてくれることがあり、人間がコードレビューをしていても気が付かなかったポイントにもコメントが入ることから、有用に感じている方も多いのではないでしょうか。
...
レビュー結果が見やすいかどうかも重要ではありますが、このコマンドの一番の嬉しいポイントとしては、一般的なコードの良し悪しを評価するAIコードレビューではなく、これまでチームが蓄積した知見を元にレビューをしてくれることになります。
...
私たちのPJでは、このようなPRには `[patch]` という接頭辞のついたPRを作ることになっており、比較的収集が用意でした。
使っているスクリプトも示しておきます。
...
補足ではありますが、 `generate_backend_reviewer_guide.sh` は常にレビュー指針をまっさらな状態から作り直し。 `generate_patch_review_guide.sh` は、件数自体そこまで多くないので、元々のレビュー指針に継ぎ足しでレビュー観点を追加しています。
...
また、エッジなケースでは、人間のコードレビューはせず、AIレビューでAIがコードを承認したり、一通りの指摘事項を修正したら、マージOKとするPJもあります。
