...
私の関わっている中規模程度のプロジェクトにおいても、とりわけバックエンド開発（Rails + GraphQL）においては、今回の手法を使い、8割〜9割ほどのコードは Cursor で生成されたものを使うことができています。

手法の一つとして、参考になる部分があれば幸いです。

## Cursor と振り返りとは？

この記事では、Cursor の Composer を用いてタスクを実行した後、そのタスクを振り返り、次回以降のタスク計画に活かすというフローをお伝えします。
...
- （エージェント）暫定的なタスクの計画を立てる
- （人間）タスク計画をレビューする
- （エージェント）承認をもらったタスクを順次遂行する
...
タスク計画書を更新する際にはエージェントにこのルールに従うよう指示をします。
...
振り返りメモを更新する際にはエージェントにこのルールに従うよう指示をします。
...
タスク計画では、Project Rules を用いて、Composer の agent モードで計画を立てさせます。
...
なお、確信度については一度計算した後、本当にその精度が正確化を疑い、厳し目に再計算してください。
...
- 確信度は、エージェントが勝手に判断します
  - 具体的なタスクを指示したとしても、エージェントが不足している点があると判断した場合には、低めの確信度がつき、追加の質問をしてくれます
- 質問に応対しながら、確信度を高めたり、不足している点や誤っている点を指摘していくことで、タスク計画をより良いものにしていきます

## Composerでのタスク実行

ここでは agent の作業を眺めることが我々の仕事になりますが、以下の点に注意してください。

- 計画から逸れてしまったり、思い通りのコードを生成しなかったとしてもなるべく手動でコードを修正せずに、エージェントに指示を出すこと
...
タスク実行時になるべくエージェントに指示を出すようにしているので、指示を出した数だけ、振り返りも実り多い時間になるはずです。
...
- 振り返りで出してきた内容は、優しい気持ちで Accept してあげましょう
  - よほど誤った解釈をしていない限り、私はエージェントの振り返りを承認しています（それが、私にとって”重要”と感じなくても）
- エージェントが出した振り返りに不足を感じる場合には、自分の言葉でもフィードバックを行い、それも振り返りメモに残しましょう
...
また、お察しの通り、このやり取りは Composer を多用するので、Premium Request を多く消費します。

私自身はあまり気にしないようにしていますが、Premium Request を節約したい方には正直あまりオススメはできません。
...
- Q: 振り返りでエージェントが出してきた内容が的を得ていないように感じる
  - A: エージェントが出してきた内容は誤りではない限り受け入れ、自分自身の観点からの補足をいれてみてください
...
昨今の生成AIでは、"記憶"を与えることで、より良いコードを出力ができる。といったような事例が見られているように思います。
